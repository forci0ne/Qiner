...
static void ctrlCHandlerRoutine(int sig)
{
    state = 1;
}
...
static void *miningThreadProc(void *ptr)
{

    const unsigned int threadIndex = *(unsigned int *)ptr;
    ...
    while (!state)
    {
        ...
        while (true)
        {
            ...
            if (neuronValues[threadIndex][NUMBER_OF_NEURONS - 1] != prevValue0 && neuronValues[threadIndex][NUMBER_OF_NEURONS - 2] == prevValue1)
            {
            ...
            {
                if (neuronValues[threadIndex][NUMBER_OF_NEURONS - 2] != prevValue1 && neuronValues[threadIndex][NUMBER_OF_NEURONS - 1] == prevValue0)
                {
                    if ((miningData[score >> 6] >> (score & 63)) & 1)
                    {
                        break;
...
static bool receiveData(int serverSocket, char *buffer, unsigned int size)
{
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                if (pthread_create(&my_thread[i], NULL, miningThreadProc, new int(i)) != 0)
                {
